...
#SBATCH --nodes=1 #2 #Number of computing nodes, e.g., six
...
#SBATCH --cpus-per-task=32 #8 #Number of CPU cores per task, e.g., 8, 16, 32
#SBATCH --ntasks-per-node=1 #4 #Number of tasks/threads per node, e.g., 4, 2, 1
#SBATCH --gpus-per-node=1 #4 ### This must be set equal to --ntasks-per-node to make each process/thread/task handle a separate GPU, thus being more efficient with PyTorch's DDP
